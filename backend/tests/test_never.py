...
def test_never_and_hard_excluded_from_selection(client, sqlite_session: Session):
    ...
    # never
    c1 = Card(word="final", definition="x"); sqlite_session.add(c1); sqlite_session.commit(); sqlite_session.refresh(c1)
    sqlite_session.add(UserCard(card_id=c1.id, bin=11, status="never")); sqlite_session.commit()

    # hard_to_remember
    c2 = Card(word="stuck", definition="y"); sqlite_session.add(c2); sqlite_session.commit(); sqlite_session.refresh(c2)
    sqlite_session.add(UserCard(card_id=c2.id, bin=1, status="hard_to_remember")); sqlite_session.commit()
