...
export default function Signup() {
  ...
  async function resend() {
    ...
    setLoading(true);
    setError(null);
    try { setMessage(await resendVerification(sentTo)); }
    catch (e) { setError(e instanceof Error ? e.message : "Could not resend email"); }
    finally { setLoading(false); }
  }

  if (sentTo) {
    return <div className="mx-auto max-w-xl py-10"><div className="game-panel p-7 text-center sm:p-10">
      <img src="/flashquest-logo.svg" alt="FlashQuest" className="mx-auto h-20 w-20 rounded-3xl" />
      <p className="metric-label mt-5">Almost there</p><h1 className="mt-2 text-3xl font-black text-white">Finish your account</h1>
      ...
      <div className="mt-5 flex flex-wrap justify-center gap-3"><button className="game-button bg-[#ffba08] px-4 py-2 text-sm font-black text-[#370617]" onClick={() => void resend()} disabled={loading}>Resend email</button><Link className="game-button border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-black text-white" to={loginTarget}>Go to sign in</Link></div>
    </div></div>;
  }

  return <div className="mx-auto grid max-w-xl gap-5 py-6 sm:py-10">
    <form onSubmit={onSubmit} className="game-panel p-6 sm:p-8">
      <div className="flex items-center gap-4"><img src="/flashquest-logo.svg" alt="FlashQuest" className="h-16 w-16 rounded-2xl" /><div><p className="metric-label">{joiningRooms ? "Join Quest Rooms" : "Continue your quest"}</p><h1 className="mt-1 text-3xl font-black text-white">Create your FlashQuest account</h1></div></div>
    ...
    <section className="game-panel border-[#faa307]/25 p-5 sm:p-6"><p className="metric-label">Want to look around first?</p><h2 className="mt-2 text-xl font-black text-white">Enter the Demo Room</h2><p className="mt-2 text-sm leading-6 text-slate-400">See the signed-in room experience before creating an account.</p>{demoError && <p className="mt-4 rounded-xl border border-[#d00000]/40 bg-[#6a040f]/40 p-3 text-sm text-rose-200">{demoError}</p>}<button type="button" className="game-button mt-4 w-full border border-[#faa307]/35 bg-[#faa307]/10 px-5 py-3 font-black text-[#ffba08]" disabled={demoLoading} onClick={() => void enterDemoRoom()}>{demoLoading ? "Entering demo…" : "👥 Enter Demo Room"}</button></section>
  </div>;
}
